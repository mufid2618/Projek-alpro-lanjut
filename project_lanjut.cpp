...
void caribuku() {
    ...
    getline(cin >> ws, judulDicari);  

    string baris, judul, deskripsi, hargaStr;
    int urutan = 1;
    bool ditemukan = false;

    while (getline(file, baris)) {
        istringstream bacabaris(baris);
        getline(bacabaris, judul, ',');
        getline(bacabaris, deskripsi, ',');
        getline(bacabaris, hargaStr);
    ...
    if (!ditemukan) {
        cout << "Buku tidak ditemukan dalam data.\n";
    }
